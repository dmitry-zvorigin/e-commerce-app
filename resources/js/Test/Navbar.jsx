...
const LinkNav = ({ name, href, icon: Icon}) => {
  return(
      <div className="ml-4 flow-root lg:ml-6">
        <InertiaLink href={href} className="-m-2 flex flex-col items-center p-2 group">
            <Icon
        ...
        <>
          {menuItemsAuth.map((item, index) => (
            <div key={index} className='flex items-center justify-center m-5'>
              <InertiaLink  method="post" href={item.href} as="button">{item.name}</InertiaLink>
            </div>
        ))}
        </>
                  ...
                  {selectedCategory.children.map((child) => (
                      <div  key={child.id} className='grid grid-cols-1'>
                        <div className='mt-2 mb-2'>
                          <InertiaLink href={route('categories', child.slug)} className='hover:text-orange-700 text-lg font-bold'>{child.name}</InertiaLink>
                        </div>
                        <hr className='mb-2'/>
                          <div className="grid grid-cols-3">
                            {child.children.map((ch) => (
                              <div className='m-1'>
                                <InertiaLink href={route('categories', ch.slug)} className='hover:text-orange-700'>{ch.name}</InertiaLink>
                              </div>
                            ))}
                          </div>
                      </div>
